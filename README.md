...
- 최신 정식 릴리스는 `v0.1.3`입니다. `develop`에는 아직 릴리스하지 않은 연구 데이터
  조회 기능(PR #44)이 포함되어 있으므로, 인수인계·데모·압축본의 기준은 항상 아래
  절차로 확인한 정확한 커밋 SHA로 남깁니다.
- `develop`에는 디자인 시스템 공통화, 설정 화면 반응형 개선, 분류 선택 UX 수정,
  Playwright 인증 E2E 테스트, Document Image 참조 이미지 관리와 연구 데이터 조회가
  후속 반영되어 있습니다.
